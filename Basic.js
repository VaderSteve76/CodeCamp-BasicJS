...
// In the code example above, the while loop will execute 5 
// times and append the numbers 0 through 4 to ourArray.
...
// condition is false at the start of the iteration, the loop will stop executing. 
...
// next condition check and is usually used to increment or decrement your loop counter.
// In the following example we initialize with i = 0 and iterate while our condition 
// i < 5 is true. We'll increment i by 1 in each loop iteration with i++ as our final-expression.
...
// For loops don't have to iterate one at a time. By 
// changing our final-expression, we can count by even numbers.
...
// not execute the code inside the loop. The result is that ourArray will 
// end up with no values added to it, and it will still look like [] when 
// all of the code in the example above has completed running. 
...
// with the while loop. When we get to the next line, there is no condition 
// to evaluate, so we go to the code inside the curly braces and execute it. 
// We will add a single element to the array and then increment i before we 
// get to the condition check. When we finally evaluate the condition i < 5 
// on the last line, we see that i is now 6, which fails the conditional check, 
// so we exit the loop and are done. At the end of the above example, the value 
// of ourArray is [5]. Essentially, a do...while loop ensures that the code 
// inside the loop will run at least once. Let's try getting a do...while loop 
// to work by pushing values to an array.
// Change the while loop in the code to a do...while loop so the loop will push 
// only the number 10 to myArray, and i will be equal to 11 when your code has finished running.
// Setup
var myArray = [];
var i = 10;
// Only change code below this line
...
// where n <= 0, it returns 1. For larger values of n, it calls itself, but 
// with n - 1. That function call is evaluated in the same way, calling multiply 
// again until n <= 0. At this point, all the functions can return and 
// the original multiply returns the answer. Note: Recursive functions must have 
// a base case when they return without calling the function again 
...
// arguments has been pre-written for you. The function should check if name 
// is an actual contact's firstName and the given property (prop) is a property of that contact.
// If both are true, then return the "value" of that property. If name does not correspond 
// to any contacts then return "No such contact". If prop does not correspond to 
// any valid properties of a contact found to match name then return "No such property".
//Setup
var contacts = [
  {
      "firstName": "Akira",
...
// random decimal number between 0 (inclusive) and not quite 
// up to 1 (exclusive). Thus Math.random() can return a 
// 0 but never quite return a 1 Note: Like Storing Values with 
// the Equal Operator, all function calls will be resolved before 
...
// Use Math.random() to generate a random decimal. Multiply that 
// random decimal by 20. Use another function, Math.floor() to round 
// the number down to its nearest whole number. Remember that Math.random() 
// can never quite return a 1 and, because we're rounding down, it's 
...
// passing the value to Math.floor() function to round the value 
// down to the nearest whole number. Use this technique to generate 
// and return a random whole number between 0 and 9.
...
// a range of two specific numbers. To do this, we'll define a minimum 
// number min and a maximum number max. Here's the formula we'll use. Take 
// a moment to read it and try to understand what this code is doing:

// Math.floor(Math.random() * (max - min + 1)) + min

// Create a function called randomRange that takes a range myMin and 
// myMax and returns a random number that's greater than or equal to 
// myMin, and is less than or equal to myMax, inclusive.
// Example
...
// Here's an example: var a = parseInt("007");
// The above function converts the string "007" to an integer 7. 
// If the first character in the string can't be converted into a 
// number, then it returns NaN. Use parseInt() in the convertToInteger 
// function so it converts the input string str into an integer, and returns it.
...
// should return either "Equal" or "Not Equal".
...
// Use multiple conditional operators in the checkSign function to 
// check if a number is positive, negative or zero. The function 
...
// call which executes the original function with different arguments. 
...
// point where n is pushed into the array, count(n - 1) has already been 
// evaluated and returned [1, 2, ..., n - 1]. We have defined a function 
// called countdown with one parameter (n). The function should use recursion 
// to return an array containing the integers n through 1 based on the n parameter. 
// If the function is called with a number less than 1, the function should return an
// empty array. For example, calling this function with n = 5 should return the 
