...
// unlike in languages such as Java, Python, Ruby, etc. In ES5, we usually 
// define a constructor function and use the new keyword to instantiate an object.
...
// new is called to create a new object. Notes: UpperCamelCase should 
// be used by convention for ES6 class names, as in SpaceShuttle used above.
// The constructor method is a special method for creating and initializing 
// an object created with a class. You will learn more about it in the Object 
...
// These are classically called getters and setters. Getter functions are meant to simply 
// return (get) the value of an object's private variable to the user without the user 
// directly accessing the private variable. Setter functions are meant to modify (set) 
// the value of an object's private variable based on the value passed into the setter 
...
// They do not even look like functions. Getters and setters 
// are important because they hide internal implementation details. 
// Note: It is convention to precede the name of a private variable 
// with an underscore (_). However, the practice itself does not make 
// a variable private. Use the class keyword to create a Thermostat 
// class. The constructor accepts a Fahrenheit temperature. Now create 
// a getter and a setter in the class, to obtain the temperature in Celsius.
// Remember that C = 5/9 * (F - 32) and F = C * 9.0 / 5 + 32, where F is the 
// value of temperature in Fahrenheit, and C is the value of the same temperature in Celsius.
// Note: When you implement this, you will track the temperature inside the class 
// in one scale, either Fahrenheit or Celsius. This is the power of a getter and 
// a setter. You are creating an API for another user, who can get the correct 
// result regardless of which one you track. In other words, you are abstracting 
...
// JavaScript started with a small role to play on an otherwise 
// mostly HTML web. Today, it’s huge, and some websites are built 
// almost entirely with JavaScript. In order to make JavaScript 
// more modular, clean, and maintainable; ES6 introduced a way to 
// easily share code among JavaScript files. This involves exporting 
// parts of a file for use in one or more other files, and importing 
// the parts you need, where you need them. In order to take advantage 
// of this functionality, you need to create a script in your 
// HTML document with a type of module. Here’s an example:

// <script type="module" src="filename.js"></script>

// A script that uses this module type can now use the import and 
// export features you will learn about in the upcoming challenges.
...
// add, that takes in two numbers and returns their sum. You want to use 
// this function in several different JavaScript files. In order to share 
// it with these other files, you first need to export it.
...
// When you export a variable or function, you can import it in 
// another file and use it without having to rewrite the code. 
// You can export multiple things by repeating the first example 
// for each thing you want to export, or by placing them all in 
// the export statement of the second example, like this:

// export { add, subtract };

// There are two string-related functions in the editor. Export 
// both of them using the method of your choice.
...
// Here, import will find add in math_functions.js, import just that function 
// for you to use, and ignore the rest. The ./ tells the import to look for 
// the math_functions.js file in the same folder as the current file. The 
// relative file path (./) and file extension (.js) are required when using 
// import in this way. You can import more than one item from the file by 
...
// in the previous lesson. These functions are in a file called string_functions.js, 
...
// the current file. This can be done with the import * as syntax. Here's an 
// example where the contents of a file named math_functions.js are imported 
...
// available for use in other files. There is another export syntax you need 
// to know, known as export default. Usually you will use this syntax if only 
// one value is being exported from a file. It is also used to 
...
// import add from "./math_functions.js"; The syntax differs in one key 
// place. The imported value, add, is not surrounded by curly braces ({}). 
...
// to make a promise to do something, usually asynchronously. When the 
...
// would use data from, to put on your website or elsewhere. Make the 
// promise handle success and failure. If responseFromServer is true, 
// call the resolve method to successfully complete the promise. Pass 
// resolve a string with the value We got the data. If responseFromServer 
// is false, use the reject method instead and pass it the string: Data not received.
const makeServerRequest = new Promise((resolve, reject) => {
...
// with the response from the server. This can be achieved by using the 
// then method. The then method is executed immediately after your 
...
// It is executed immediately after a promise's reject method 
// is called. Here’s the syntax:
...
// Note: the then and catch methods can be chained to the 
// promise declaration if you choose. Add the catch method 
// to your promise. Use error as the parameter of its callback 
