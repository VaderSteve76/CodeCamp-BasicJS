...
// to search for patterns at the beginning of strings. There is 
// also a way to search for patterns at the end of strings.
...
// Using character classes, you were able to search for all letters 
// of the alphabet with [a-z]. This kind of character class is common 
// enough that there is a shortcut for it, although it includes a few 
// extra characters as well. The closest character class in JavaScript 
// to match the alphabet is \w. This shortcut is equal to [A-Za-z0-9_]. 
...
// These shortcut character classes are also known 
// as shorthand character classes. Use the shorthand 
// character class \w to count the number of alphanumeric 
...
// for is the opposite of alphanumerics. You can search for the opposite 
// of the \w with \W. Note, the opposite pattern uses a capital letter. 
...
// \d with a lowercase d. You can also search for non-digits using a 
// similar shortcut that uses an uppercase D instead. The shortcut to 
// look for non-digit characters is \D. This is equal to the character 
// class [^0-9], which looks for a single character that is not a number 
// between zero and nine. Use the shorthand character class for non-digits 
...
// Usernames are used everywhere on the internet. They are what 
// give users a unique identity on their favorite sites.
...
// 2) The only numbers in the username have to be at the end. There 
// can be zero or more of them at the end. Username cannot start with the number.
...
// spaces between letters. You can search for whitespace using 
// \s, which is a lowercase s. This pattern not only matches 
// whitespace, but also carriage return, tab, form feed, and 
// new line characters. You can think of it as similar to the 
// character class [ \r\t\f\n\v].
...
// lowercase s. You can also search for everything except whitespace.
// Search for non-whitespace using \S, which is an uppercase s. This 
// pattern will not match whitespace, carriage return, tab, form 
// feed, and new line characters. You can think of it being similar 
...
// characters and the asterisk * to look for zero or more characters. 
// These are convenient but sometimes you want to match a certain 
// range of patterns. You can specify the lower and upper number of 
// patterns with quantity specifiers. Quantity specifiers are used with 
// curly brackets ({ and }). You put two numbers between the curly 
// brackets - for the lower and upper number of patterns.
// For example, to match only the letter a appearing between 3 and 5 
// times in the string "ah", your regex would be /a{3,5}h/.
...
// number followed by a comma. For example, to match only the 
// string "hah" with the letter a appearing at least 3 times, 
...
// with quantity specifiers using curly brackets. Sometimes 
// you only want a specific number of matches. To specify a 
// certain number of patterns, just have that one number 
// between the curly brackets. For example, to match only 
// the word "hah" with the letter a 3 times, your regex 
// would be /ha{3}h/.
...
// Change the regex timRegex to match the word 
// "Timber" only when it has four letter m's.
...
// ( and ), are used to find repeat substrings. You put the regex of the 
// pattern that will repeat in between the parentheses. To specify where 
// that repeat string will appear, you use a backslash (\) and then a number. 
// This number starts at 1 and increases with each additional capture group 
// you use. An example would be \1 to match the first group.
...
// string it matches, along with its capture group. Use capture groups 
// in reRegex to match numbers that are repeated only three times in a 
...
// when it also changes (or replaces) the text you match. You can search and 
// replace text in a string using .replace() on a string. The inputs for .replace() 
// is first the regex pattern you want to search for. The second parameter is the 
...
// the string "one two three". Then update the replaceText variable to replace 
...
// Sometimes whitespace characters around strings are not wanted 
// but are there. Typical processing of strings is to remove the 
// whitespace at the start and end of it. Write a regex and use 
// the appropriate string methods to remove whitespace at the 
// beginning and end of strings. Note: The String.prototype.trim() 
// method would work here, but you'll need to complete this 
